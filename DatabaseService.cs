...
";
    ...
    private void SeedDataIfEmpty(SqliteConnection connection)
    {
        ...
        if (productCount == 0)
        {
            var products = new[]
            {
                new Product { Sku = "PICE-001", Barcode = "8600000000011", Name = "Gazirani sok 1.5L", CategoryId = 1, Unit = "kom", PurchasePrice = 92, SalePrice = 139, StockQuantity = 46, MinStock = 12 },
                new Product { Sku = "PICE-002", Barcode = "8600000000028", Name = "Mineralna voda 1.5L", CategoryId = 1, Unit = "kom", PurchasePrice = 38, SalePrice = 69, StockQuantity = 8, MinStock = 20 },
                new Product { Sku = "MLEK-001", Barcode = "8600000000035", Name = "Mleko 2.8% 1L", CategoryId = 2, Unit = "kom", PurchasePrice = 104, SalePrice = 149, StockQuantity = 31, MinStock = 15 },
                new Product { Sku = "PEKA-001", Barcode = "8600000000042", Name = "Hleb beli 500g", CategoryId = 3, Unit = "kom", PurchasePrice = 54, SalePrice = 89, StockQuantity = 18, MinStock = 10 },
                new Product { Sku = "SLAT-001", Barcode = "8600000000059", Name = "Čokolada 100g", CategoryId = 4, Unit = "kom", PurchasePrice = 79, SalePrice = 129, StockQuantity = 64, MinStock = 18 },
                new Product { Sku = "HEMI-001", Barcode = "8600000000066", Name = "Deterdžent 2kg", CategoryId = 5, Unit = "kom", PurchasePrice = 420, SalePrice = 599, StockQuantity = 11, MinStock = 6 }
            };

            foreach (var product in products)
            {
                var id = InsertProduct(connection, product);
...
VALUES ($productId, 'Početno stanje', $quantity, $unitCost, 'Demo lager za portfolio projekat', $createdAt);";
                movement.Parameters.AddWithValue("$productId", id);
                movement.Parameters.AddWithValue("$quantity", product.StockQuantity);
                movement.Parameters.AddWithValue("$unitCost", product.PurchasePrice);
                movement.Parameters.AddWithValue("$createdAt", DateTime.Now.ToString("O"));
...
SELECT $id;";
    ...
    private int InsertProduct(SqliteConnection connection, Product product)
    {
        using var command = connection.CreateCommand();
        command.CommandText = @"
...
VALUES ($productId, $type, $quantity, $unitCost, $note, $createdAt);";
        movement.Parameters.AddWithValue("$productId", productId);
        movement.Parameters.AddWithValue("$type", type);
        movement.Parameters.AddWithValue("$quantity", signedQuantity);
        movement.Parameters.AddWithValue("$unitCost", unitCost);
    ...
    public int CreateSale(List<SaleCartItem> cartItems)
    {
        ...
        var receiptNumber = $"MSM-{DateTime.Now:yyyyMMdd-HHmmss}";
...
SELECT last_insert_rowid();";
        ...
        saleCommand.Parameters.AddWithValue("$createdAt", DateTime.Now.ToString("O"));
...
INSERT INTO StockMovements (ProductId, Type, Quantity, UnitCost, Note, CreatedAt)
VALUES ($productId, 'Prodaja', $quantity, 0, $note, $createdAt);";
            ...
            movementCommand.Parameters.AddWithValue("$note", $"Račun {receiptNumber}");
            movementCommand.Parameters.AddWithValue("$createdAt", DateTime.Now.ToString("O"));
...
LIMIT $limit;";
        ...
        while (reader.Read())
        {
            movements.Add(new StockMovement
            {
                ...
                CreatedAt = DateTime.Parse(reader["CreatedAt"].ToString() ?? DateTime.Now.ToString("O"))
            });
        }
...
LIMIT $limit;";
    ...
    {
        using var connection = CreateConnection();
        var todayStart = DateTime.Today.ToString("O");
        var tomorrowStart = DateTime.Today.AddDays(1).ToString("O");
